...
if not BOT_TOKEN:
    raise RuntimeError("Не задан BOT_TOKEN. Создайте бота через @BotFather и задайте переменную окружения BOT_TOKEN.")
...
def normalize(text: str) -> str:
    ...
    text = re.sub(r"\([^)]*\)", " ", text)
    text = re.sub(r"\b(улица|ул|проспект|пр-т|переулок|пер|площадь|пл)\b", " ", text)
    text = re.sub(r"[^а-яa-z0-9]+", " ", text)
    return re.sub(r"\s+", " ", text).strip()

NAMES = [x["name"] for x in STREETS]
NORMALIZED = {x["name"]: normalize(x["name"]) for x in STREETS}
...
def find_matches(query: str, limit: int = 5):
    ...
    for item in STREETS:
        name = item["name"]
        n = NORMALIZED[name]
        score = fuzz.WRatio(q, n)
        if q in n or n in q:
            score = max(score, 92)
        scored.append((score, item))

    scored.sort(key=lambda x: x[0], reverse=True)
    return [(score, item) for score, item in scored[:limit] if score >= 48]

def clean_description(text: str) -> str:
    """Убирает технические пометки из таблицы, не меняя основной исторический текст."""
    text = text or ""
...
def format_entry(item: dict) -> str:
    """Формирует читабельный связный ответ вместо структуры исходной таблицы."""
    name = escape(item.get("name", "").strip())
    district = escape(item.get("district", "").strip())
    description = escape(clean_description(item.get("description", "")))
...
async def count_command(message: Message):
    await message.answer(f"В базе бота: <b>{len(STREETS)}</b> записей из предоставленной таблицы.")
...
async def street_search(message: Message):
    ...
    if not matches:
        await message.answer(
            ...
            "Попробуйте ввести только основную часть названия, например «Гастелло» или «Панфилова»."
        )
        return

    best_score, best = matches[0]
    # Exact/very strong match: immediately show the record.
    if best_score >= 88:
